...
def threshold_output():
    ...
    print(f"Threshold Output: {output}")
...
def next_iteration():
    ...
    if [X[current_iteration, 0], X[current_iteration, 1]] == [0, 0]:
        print(f"sequence_error = {sequence_error}")
        if sequence_error != 0:
            iteration_counter += 1
            window.update()
            iteration_counter_label.config(text=f"Epoch: {iteration_counter}")
            print(f"Epoch: {iteration_counter}")
            window.update()
            sequence_error = 0
            sequence_error_label.config(text="Total Error: 0")
        else:
...
updated_weights_label = tk.Label(window, text="Weights: [0.706, 0.533, 0.58]\n")
